...
```{r}
#| output: asis
round0 <- function(x) floor(as.numeric(x) + 0.5)
vals <- df_mmr |>
  dplyr::filter(geography %in% c(state_title, "United States")) |>
  dplyr::mutate(estimate_percent = suppressWarnings(as.numeric(estimate_percent)))

vals_state    <- vals |> dplyr::filter(geography == state_title) |> dplyr::pull(estimate_percent)
vals_national <- vals |> dplyr::filter(geography == "United States") |> dplyr::pull(estimate_percent)

# 1) State vs national — if rounded values match, say "similar to"
st_r <- round0(vals_state)
us_r <- round0(vals_national)
label_national_comparison <-
  if (is.na(st_r) || is.na(us_r)) "similar to" else
  if (st_r > us_r) "above" else
  if (st_r < us_r) "below" else
  "similar to"
